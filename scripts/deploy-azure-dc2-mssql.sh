...
echo "=== WSO2 APIM 4.7 — Azure DC2 (West US 2) Deployment [MSSQL] ==="
...
if kubectl get secret apim-ingress-tls -n "$NAMESPACE" &>/dev/null; then
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
        ...
        -addext "subjectAltName=DNS:cp.wus2.apim.example.com,DNS:gw.wus2.apim.example.com" 2>/dev/null
...
helm install cp "$REPO_DIR/distributed/control-plane" -n "$NAMESPACE" \
    -f "$REPO_DIR/distributed/control-plane/azure-values-dc2-mssql.yaml"
...
helm install tm "$REPO_DIR/distributed/traffic-manager" -n "$NAMESPACE" \
    -f "$REPO_DIR/distributed/traffic-manager/azure-values-dc2-mssql.yaml"
...
helm install gw "$REPO_DIR/distributed/gateway" -n "$NAMESPACE" \
    -f "$REPO_DIR/distributed/gateway/azure-values-dc2-mssql.yaml"
...
echo ""
echo "========================================="
echo "  DC2 Deployment Complete [MSSQL]"
echo "========================================="
echo ""
...
echo ""
echo "Next step: Run ./scripts/setup-cross-dc-mssql.sh (after both DCs are deployed)"
