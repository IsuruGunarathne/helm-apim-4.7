...
echo ""
echo "Verify cross-DC connectivity:"
echo "  # From DC1, test connection to DC2"
...
echo "    -c wso2am-control-plane -- curl -sk https://${DC2_ILB_IP}:9443/services/Version"
echo ""
echo "  # From DC2, test connection to DC1"
echo "  kubectl config use-context $DC2_CONTEXT"
echo "  kubectl exec -n apim \$(kubectl get pod -l deployment=wso2am-cp -n apim -o jsonpath='{.items[0].metadata.name}') \\"
echo "    -c wso2am-control-plane -- curl -sk https://${DC1_ILB_IP}:9443/services/Version"
