...
echo "=== WSO2 APIM 4.7 — Azure DC1 (East US 1) Deployment [Oracle] ==="
...
helm install cp "$REPO_DIR/distributed/control-plane" -n "$NAMESPACE" \
    -f "$REPO_DIR/distributed/control-plane/azure-values-dc1-oracle.yaml"
...
helm install tm "$REPO_DIR/distributed/traffic-manager" -n "$NAMESPACE" \
    -f "$REPO_DIR/distributed/traffic-manager/azure-values-dc1-oracle.yaml"
...
helm install gw "$REPO_DIR/distributed/gateway" -n "$NAMESPACE" \
    -f "$REPO_DIR/distributed/gateway/azure-values-dc1-oracle.yaml"
...
echo ""
echo "========================================="
echo "  DC1 Deployment Complete [Oracle]"
echo "========================================="
echo ""
...
echo ""
echo "Next step: Deploy DC2, then run ./scripts/setup-cross-dc-oracle.sh"
