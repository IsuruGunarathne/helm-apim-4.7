...
import org.wso2.am.admin.clients.application.CarbonApplicationUploaderClient;
...
public class APIMIntegrationBaseTest {
    ...
    protected CarbonApplicationUploaderClient carbonApplicationUploaderClient;
        ...
        try {
            ...
            restAPIInternal =
                    new RestAPIInternalImpl(superTenantKeyManagerContext.getContextTenant().getContextUser().getUserName(),
                            ...
                            user.getUserDomain(), keyMangerUrl.getWebAppURLHttps());
            ...
            restAPIServiceCatalog = new RestAPIServiceCatalogImpl(user.getUserName(), user.getPassword(), user.getUserDomain(), publisherURLHttps);
            restAPIStore =
            ...
            try {
                ...
                userManagementClient = new UserManagementClient(
                        getHttpBackendUrl(keyManagerContext), keymanagerSessionCookie);
                ...
                tenantManagementServiceClient = new TenantManagementServiceClient(
                        getHttpBackendUrl(superTenantKeyManagerContext),
                        keymanagerSuperTenantSessionCookie);
                remoteClaimMetaDataMgtAdminClient =
                        new ClaimMetaDataMgtAdminClient(getHttpBackendUrl(keyManagerContext),
                                keymanagerSessionCookie);
                identityProviderMgtClient =
                        new IdentityProviderMgtClient(getHttpBackendUrl(keyManagerContext),
                                keymanagerSessionCookie);
                oAuthAdminServiceClient =
                        new OAuthAdminServiceClient(getHttpBackendUrl(keyManagerContext),
                                keymanagerSessionCookie);
                applicationManagementClient =
                        new ApplicationManagementClient(getHttpBackendUrl(keyManagerContext),
                                keymanagerSessionCookie);
                identityProviderMgtServiceClient =
                        new IdentityProviderMgtServiceClient(getHttpBackendUrl(keyManagerContext),
                                keymanagerSessionCookie);
                carbonApplicationUploaderClient =
                        new CarbonApplicationUploaderClient(getHttpBackendUrl(keyManagerContext),
                                keymanagerSessionCookie);
            } catch (Exception e) {
    ...
    protected void updateSynapseConfiguration(OMElement synapseConfig,
        ...
        try {
            ...
            APIMTestCaseUtils.updateSynapseConfiguration(synapseConfig,
                                                         getHttpBackendUrl(automationContext),
                                                         sessionCookie);
    ...
    protected String getKeyManagerURLHttps() throws XPathExpressionException {
        return getHttpBackendUrl(keyManagerContext).replace("/services", "");
    }

    /**
     * Get HTTP backend URL for SOAP admin services. Uses the HTTP web app URL
     * instead of the HTTPS backend URL to avoid TLS issues with port-forward.
     */
    protected String getHttpBackendUrl(AutomationContext context) throws XPathExpressionException {
        return context.getContextUrls().getWebAppURL() + "/services/";
    }
