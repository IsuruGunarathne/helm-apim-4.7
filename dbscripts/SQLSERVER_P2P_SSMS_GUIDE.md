...
## Step 6a: Deploy Replication Stored Procedures to DC2

When Step 7 runs with **"I created the peer database manually"**, the subscription is created with `@sync_type = 'replication support only'`. No snapshot is shipped, so SQL Server assumes DC2 already contains the per-article `sp_MSins_*` / `sp_MSupd_*` / `sp_MSdel_*` custom procedures that the Distribution Agent calls on every applied row. Step 5 only created the base tables, so these procs exist on DC1 (auto-generated by `sp_addarticle` in Step 6) but are absent on DC2.

> We can't initialize DC2 from a DC1 backup either — that would overwrite DC2's `IDENTITY(2,2)` seeds with DC1's odd-numbered ones. Scripting the procs is the only path that preserves the odd/even identity split.

On **DC1**, open a New Query and run for `apim_db`:

```sql
USE apim_db;
EXEC sp_scriptpublicationcustomprocs @publication = 'apim_db_pub';
GO
```

Click the result set, copy the entire output (a large block of `CREATE PROCEDURE` statements), connect to **DC2**, switch the query to `apim_db`, and execute it.

Repeat for `shared_db`:

```sql
USE shared_db;
EXEC sp_scriptpublicationcustomprocs @publication = 'shared_db_pub';
GO
```

Copy the output and execute it against `shared_db` on DC2.

Verify on DC2:

```sql
USE apim_db;
SELECT COUNT(*) FROM sys.procedures WHERE name LIKE 'sp_MSins_%';
-- Should match the number of tables published
```

> Must run **after** Step 6 (procs only exist on DC1 once articles are added) and **before** Step 7 (the Distribution Agent calls these the moment the first transaction arrives).
...
| `Could not find stored procedure 'dbo.sp_MSins_...'` (or `dbo.i_<Table><hash>`) | DC2 is missing the per-article custom replication procs that DC1 auto-generated in Step 6 | Run Step 6a to script the procs from DC1 onto DC2. **Do not** "fix" this by dropping `@status = 16` from the articles — that silently switches replication to raw DML and disables P2P conflict detection, which runs inside those custom procs |
...
| 6 | Create publications (SSMS wizard) | `apim_db_pub` + `shared_db_pub`, P2P enabled | — (created by Step 7 wizard) |
| 7 | P2P Topology Wizard | Adds DC2 peer, creates both subscriptions | Pub + subscriptions created by wizard |
